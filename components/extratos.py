...
layout = dbc.Col([
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(id='bar-graph', style={'marginRight': '20px'})
        ], width=9),
        ...
        dbc.Col([
            dbc.Card(
                dbc.CardBody([
                    html.H4('Despesas'),
                    html.Legend('R$ 4430,09', id='valor_despesa_card', style={'font-size': '60px'}),
                    html.H6("Total de despesas")
                ], style={'text-align': 'center', 'paddingTop': '30px'})
            )
        ], width=3)
    ])
], style={'padding': '10px'})
